...
class BufferPool:
    ...
    def get_page(self, page_key, load_fn):
        ...
        self.lru_order[page_key] = True  # Add to LRU (will be at end)
        self.lru_order.move_to_end(page_key)
    ...
    def unpin(self, page_key, is_dirty=False):
        if page_key not in self.frames:
            return  # Page may have already been evicted (shouldn't happen in normal use)
    ...
    def flush_all(self, write_fn):
        ...
        """
        for page_key, frame in self.frames.items():
            if frame.is_dirty:
    ...
    def _evict(self):
        ...
        """
        # Walk LRU from front (least recently used) to find an unpinned page
        for page_key in self.lru_order:
            frame = self.frames[page_key]
            if frame.pin_count == 0:
                # Found a victim
                if frame.is_dirty:
                    # We need a write function here — store it at pool level
                    # This is called only if flush callbacks are registered
    ...
    # Initialize write callback to None
    _write_fn = None



def make_page_key(table_name, is_tail, page_id, col_index):
    return (table_name, is_tail, page_id, col_index)


def parse_page_key(page_key):
    return page_key  # already a tuple, just return for clarity